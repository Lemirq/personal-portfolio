...
export default function CardsSection({ projects, onViewChange }: CardsSectionProps) {
  ...
  return (
    <div ref={containerRef} className="relative h-[400vh] w-full">
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        <div className="w-full max-w-7xl mx-auto pt-16 md:pt-32 px-5 md:px-10 absolute z-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4">
          <motion.h2
          ...
          {onViewChange && (
            <motion.button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-colors text-sm text-white/70 hover:text-white pointer-events-auto"
            >
...
function Carousel({ projects }: { projects: any[] }) {
  ...
  const radius = Math.max(1.4, count * 0.27);
  ...
  return (
    <>
      {projects?.map((project, i) => {
        ...
        const galleryItem = project.gallery?.find(
        ...
        if (galleryItem) {
          try {
            ...
            console.warn(
              "Could not generate image url for project",
              project.title,
              e,
            );
          }
