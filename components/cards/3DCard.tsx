...
function CardMesh({ url, hovered, geometry }: { url: string, hovered: boolean, geometry: THREE.BufferGeometry }) {
  const ref = useRef<any>(null)
  ...
  useFrame((state: any, delta: number) => {
    if (ref.current) {
        ...
        if (ref.current.material.radius !== undefined) {
             easing.damp(ref.current.material, 'radius', hovered ? 0.2 : 0.1, 0.2, delta)
        }
    }
  })
